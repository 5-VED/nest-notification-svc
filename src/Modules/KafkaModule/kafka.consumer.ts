...
export class KafkaConsumer {
  ...
  private async handleBulkNotifications(eventData: any) {
    ...
    this.logger.log(`Processing bulk notifications batch ${batchId}: ${bulkNotifications.length} notifications`);
    ...
    try {
      ...
      const successCount = results.filter(r => r.status === 'fulfilled').length * batchSize;
      const throughput = Math.round((bulkNotifications.length / processingTime) * 1000);

      this.logger.log(`Bulk processing completed: ${successCount}/${bulkNotifications.length} notifications in ${processingTime}ms (${throughput}/sec)`);
