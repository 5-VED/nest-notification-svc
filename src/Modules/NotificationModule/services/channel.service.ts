...
export class ChannelService {
    ...
    constructor(private readonly prisma: PrismaService, private readonly templateRenderer: TemplateRenderer) { }
    ...
    async getDeviceTokens(userId: string): Promise<string[]> {
        try {
            ...
            this.logger.error(`Error fetching device tokens for user ${userId}:`, error);
            return [];
        }
    }
    ...
    async updateUserPreferences(userId: string, channel: string, enabled: boolean) {
        try {
    ...
    async deactivateDeviceToken(userId: string, token: string) {
        try {
            ...
            this.logger.error(`Error deactivating device token for user ${userId}:`, error);
            throw error;
        }
    }
    ...
    async getNotificationTemplate(type: string, channel: string) {
        try {
            ...
            this.logger.error(`Error fetching template for ${type}/${channel}:`, error);
            return null;
        }
    }

    async renderTemplate(template: any, variables: Record<string, any>): Promise<{ title: string; message: string; htmlContent?: string }> {
